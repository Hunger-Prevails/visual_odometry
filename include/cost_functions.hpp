...
class ProjectionErrorTransform {
protected:
    Eigen::Vector2d point;
    Eigen::Matrix3d intrinsics;
...
public:
    ProjectionErrorTransform(
        const cv::Point2f& point,
        const Eigen::Matrix3d& intrinsics,
        const Eigen::Vector3d& landmark
    ):
        point({point.x, point.y}), intrinsics(intrinsics), landmark(landmark) {}
    ...
    bool operator()(
        ...
        auto landmark = this->landmark.cast<T>();
